...
public class AnalysisExecutionLogFile extends FileWriterHelper {
	 ...
	 */
	public AnalysisExecutionLogFile(Path filePath) {
		super(filePath);
	 ...
	 * @return Log file
	 */
	public static AnalysisExecutionLogFile newInstance(String fileName) {
		AnalysisExecutionLogFile result = null;
		try
		{

			Path analysisPath = CommonUtils.getLogPath();
			result = new AnalysisExecutionLogFile(analysisPath.resolve(fileName.replace("{docType}", "analysisLog")));
		}
		catch(Exception ex) {
			LogFactory.getLog(AnalysisExecutionLogFile.class).error(ex.getMessage(), ex);
		}
