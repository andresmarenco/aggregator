...
public class BasicMaxVerticalSelection extends AbstractSelectionModel {
	
	/**
	 * Default Constructor
	 * @param collection Vertical Collection
	 */
	public BasicMaxVerticalSelection(VerticalCollection collection) {
		super(collection);
	}
	...
	public List<Entry<String, Double>> executeModel(String queryString) {
		...
		try
		{
			...
			for(Map.Entry<String, CSIVertical> data : csiResult.getVerticals().entrySet()) {
				result.add(new ImmutablePair<String, Double>(data.getKey(), data.getValue().getScoresMax()));
			}
		}
		catch(Exception ex) {
